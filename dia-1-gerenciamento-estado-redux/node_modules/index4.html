...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      const actionPrimeiro = () => {
        store.dispatch({type: 'ALTERANDO_PRIMEIRO'})
      }

      const actionSegundo = () => {
        store.dispatch({type: 'ALTERANDO_SEGUNDO'})
      }

      const meuPrimeiroReducer = (state = ESTADO_INICIAL_1, action) => {
        switch (action.type) {
          case 'ALTERANDO_PRIMEIRO':
...
// Crie dispatchs para alterar os nomes após alguns segundos depois que a página carregar.
    window.onload = () => {
      setTimeout(() => {
        store.dispatch({
