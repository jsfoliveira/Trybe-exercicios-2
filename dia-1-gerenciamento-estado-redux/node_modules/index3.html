...
<html lang="en">
  ...
  <body>
    ...
    const actionColors = () => {
      store.dispatch({ type: 'RANDOM_COLORS' });
    };
...
// Não se esqueça de colocar o reducer como parâmetro para o createStore
...
// Crie um botão com o texto Random color , um eventListener e uma action no reducer adicionando uma cor aleatória ao array colors guardado em nosso estado inicial. Faça também com que essa cor seja mostrada ao ser criada, alterando o estado index para a posição dela
const btnRondom = document.getElementById("previous");
btnRondom.addEventListener("click", actionColors);
...
btnNext.addEventListener("click", actionNextColor);
btnPrevius.addEventListener("click", actionPreviousColor);
